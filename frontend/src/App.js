...
import { TrendingUp, Activity, BarChart3, DollarSign, Download, ChevronDown, ChevronUp, PieChart } from 'lucide-react';
...
function App() {
  ...
  const downloadReport = () => {
    ...
    const dcf = results.dcf_analysis;
    const revenue = results.revenue_forecast;
    const comps = results.comparable_companies;
    const ml = results.ml_synthesis;
...
Analysis Date: ${results.analysis_date || new Date().toLocaleDateString()}

CURRENT PRICE: $${summary?.current_price?.toFixed(2) || 'N/A'}
TARGET PRICE: $${summary?.consensus_target?.toFixed(2) || 'N/A'}
RECOMMENDATION: ${summary?.recommendation?.action || 'N/A'}
UPSIDE POTENTIAL: ${((summary?.recommendation?.upside_potential || 0) * 100).toFixed(1)}%
...
DCF VALUATION
--------------------------------------------------------------------------------
Intrinsic Value: $${dcf?.valuation?.intrinsic_value_per_share?.toFixed(2) || 'N/A'}
WACC: ${((dcf?.discount_rate?.wacc || 0) * 100).toFixed(2)}%
Premium/(Discount): ${((dcf?.valuation?.premium_discount || 0) * 100).toFixed(1)}%
Assessment: ${dcf?.recommendation || 'N/A'}

REVENUE FORECAST
--------------------------------------------------------------------------------
Revenue CAGR: ${((revenue?.growth_analysis?.cagr || 0) * 100).toFixed(1)}%
Growth Trend: ${revenue?.growth_analysis?.trend || 'N/A'}
Current Revenue: $${((revenue?.forecast?.current_revenue || 0) / 1e9).toFixed(2)}B
Assessment: ${revenue?.recommendation || 'N/A'}

COMPARABLE COMPANIES
--------------------------------------------------------------------------------
Peer Group: ${comps?.peer_group?.join(', ') || 'N/A'}
Implied Price: $${comps?.valuation?.overall_assessment?.average_implied_price?.toFixed(2) || 'N/A'}
Relative Valuation: ${((comps?.valuation?.overall_assessment?.overall_upside_downside || 0) * 100).toFixed(1)}%
Assessment: ${comps?.recommendation || 'N/A'}

================================================================================
                        ML SYNTHESIS
================================================================================
ML Score: ${ml?.ml_score?.toFixed(3) || 'N/A'} (Scale: -1 Sell to +1 Buy)
Model Consensus Target: $${ml?.target_price?.toFixed(2) || 'N/A'}

PRICE SCENARIOS
--------------------------------------------------------------------------------
Bull Case: $${summary?.recommendation?.price_range?.bull_case?.toFixed(2) || 'N/A'}
Base Case: $${summary?.recommendation?.price_range?.base_case?.toFixed(2) || 'N/A'}
Bear Case: $${summary?.recommendation?.price_range?.bear_case?.toFixed(2) || 'N/A'}

================================================================================
                        INVESTMENT THESIS
================================================================================
${ml?.comprehensive_analysis || 'N/A'}
...
Report generated by doDiligence - ${new Date().toLocaleString()}
================================================================================
`;
    ...
    a.href = url;
    a.download = `${results.ticker}_Analysis_Report_${new Date().toISOString().split('T')[0]}.txt`;
  ...
  const styles = `
    ...
    * {
    ...
    /* Chart Bars */
    .chart-bar {
      background: linear-gradient(180deg, rgba(0,187,255,0.5), rgba(0,187,255,0.2));
      border-radius: 4px;
      position: relative;
      overflow: hidden;
          ...
          {results && results.dcf_analysis && (
            <div className="dashboard-grid" style={{
                            ...
                            <div key={idx} style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                              <div
                                ...
                                style={{
                                  height: `${height}%`,
                                  minHeight: '10px',
                                  background: 'linear-gradient(180deg, rgba(0,255,136,0.5), rgba(0,255,136,0.2))'
                                }}
                              />
                              <div style={{ fontSize: '10px', opacity: 0.4, marginTop: '4px', textAlign: 'center' }}>
                                Y{idx + 1}
              ...
              <div className="liquid-glass-card" style={{ gridColumn: '1 / -1' }}>
                <div className="glass-content">
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
                    <PieChart size={24} style={{ opacity: 0.7 }} />
                    <h3 style={{ fontSize: '18px', fontWeight: '700', letterSpacing: '1px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '40px', flexWrap: 'wrap' }}>
                    ...
                    <div style={{ flex: '1', minWidth: '300px' }}>
                      <div style={{ marginBottom: '20px' }}>
                        {[
                          { label: 'Current Price', value: results.executive_summary.current_price, color: '#ffffff' },
                          { label: 'DCF Value', value: results.dcf_analysis.valuation?.intrinsic_value_per_share, color: '#00bbff' },
                          { label: 'Comps Value', value: results.comparable_companies.valuation?.overall_assessment?.average_implied_price, color: '#ff44ff' },
                          { label: 'Target Price', value: results.executive_summary.consensus_target, color: '#00ff88' }
                        ].map((item, idx) => {
                          const maxVal = Math.max(
                            results.executive_summary.current_price || 0,
                            results.dcf_analysis.valuation?.intrinsic_value_per_share || 0,
                            results.comparable_companies.valuation?.overall_assessment?.average_implied_price || 0,
                            results.executive_summary.consensus_target || 0
                          );
                          const width = maxVal > 0 ? ((item.value || 0) / maxVal) * 100 : 0;
                          return (
                    ...
                    <div style={{
                      ...
                      {[
                        { label: 'DCF Upside', value: results.executive_summary.key_metrics?.dcf_upside, format: 'percent' },
                        { label: 'Revenue CAGR', value: results.executive_summary.key_metrics?.revenue_cagr, format: 'percent' },
                        { label: 'Comps Upside', value: results.executive_summary.key_metrics?.comps_upside, format: 'percent' },
                        { label: 'ML Score', value: results.executive_summary.key_metrics?.ml_score, format: 'score' }
                      ].map((metric, idx) => (
                        <div key={idx} style={{
                          padding: '16px',
                          ...
                          <div style={{
                            fontSize: '18px',
                            fontWeight: '700',
                            color: metric.format === 'score'
                              ? (metric.value > 0.2 ? '#00ff88' : metric.value < -0.2 ? '#ff3333' : '#ffaa00')
                              : (metric.value > 0 ? '#00ff88' : '#ff3333')
                          }}>
                ...
                <div className="glass-content">
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      {showFullAnalysis ? 'COLLAPSE' : 'EXPAND FULL ANALYSIS'}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px' }}>
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                        <div style={{
                          ...
                          height: '8px',
                          borderRadius: '50%',
                          background: results.dcf_analysis.valuation?.premium_discount > 0 ? '#00ff88' : '#ff3333',
                          marginTop: '6px',
                          flexShrink: 0
                        }} />
                        <div style={{ fontSize: '13px', opacity: 0.8, lineHeight: '1.6' }}>
                          DCF analysis suggests stock is {results.dcf_analysis.valuation?.premium_discount > 0 ? 'undervalued' : 'overvalued'} by {Math.abs((results.dcf_analysis.valuation?.premium_discount || 0) * 100).toFixed(1)}%
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                        <div style={{
                          ...
                          height: '8px',
                          borderRadius: '50%',
                          background: (results.revenue_forecast.growth_analysis?.cagr || 0) > 0.05 ? '#00ff88' : '#ffaa00',
                          marginTop: '6px',
                          flexShrink: 0
                        }} />
                        <div style={{ fontSize: '13px', opacity: 0.8, lineHeight: '1.6' }}>
                          Revenue growing at {((results.revenue_forecast.growth_analysis?.cagr || 0) * 100).toFixed(1)}% CAGR with {results.revenue_forecast.growth_analysis?.trend || 'stable'} trend
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                        <div style={{
                          ...
                          height: '8px',
                          borderRadius: '50%',
                          background: (results.comparable_companies.valuation?.overall_assessment?.overall_upside_downside || 0) > 0 ? '#00ff88' : '#ff3333',
                          marginTop: '6px',
                          flexShrink: 0
                        }} />
                        <div style={{ fontSize: '13px', opacity: 0.8, lineHeight: '1.6' }}>
                          Trading at {(results.comparable_companies.valuation?.overall_assessment?.overall_upside_downside || 0) > 0 ? 'discount' : 'premium'} to peer group median
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                        <div style={{
                          ...
                          height: '8px',
                          borderRadius: '50%',
                          background: (results.ml_synthesis.ml_score || 0) > 0.2 ? '#00ff88' : (results.ml_synthesis.ml_score || 0) < -0.2 ? '#ff3333' : '#ffaa00',
                          marginTop: '6px',
                          flexShrink: 0
                        }} />
                        <div style={{ fontSize: '13px', opacity: 0.8, lineHeight: '1.6' }}>
                          ML model rates this a {(results.ml_synthesis.ml_score || 0) > 0.2 ? 'buy' : (results.ml_synthesis.ml_score || 0) < -0.2 ? 'sell' : 'hold'} with {Math.abs(results.ml_synthesis.ml_score || 0).toFixed(2)} confidence
                        </div>
                      </div>
                    </div>
                  </div>
